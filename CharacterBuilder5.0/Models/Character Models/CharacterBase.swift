...
class CharacterBase: Codable {
	...
	var charFeatsFull: FeatList = PublicLists().featList
	...
	func addElement(targetElement selection: LevelListItem, elementType: NavigationMenuItem.MenuName, level: Int){
		...
		case .featList:
			let featType = selection.featReqType
			for type in LevelListItem.FeatPrereqType.allCases {
				if type == featType {
					
				}
			}
			charFeats?.addListItem(selection)
			charFeatsFull.toggleFeatFromSelection(selection: selection)
		case .backgrounds:
	...
	func deleteElement(targetElement selection: LevelListItem, elementType: NavigationMenuItem.MenuName){
		...
		case .featList:
			...
			charFeatsFull.setFeatSelection(selection: selection, set: false)
		case .backgrounds:
